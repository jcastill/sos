...
class Upload():
    ...
    # TL;DR Use _upload_* for profile default values, use upload_* when wanting
    ...
    def pre_work(self, hook_commons):
        ...
        # The profile default '_' prefixed versions of these are untouched to
        # allow fallback
        ...
        if not cmdline_opts.batch and not \
                cmdline_opts.quiet:
            # Profiles will need to handle the prompts for user information
    ...
    def prompt_for_upload_s3_access_key(self):
        """Should be overridden by profiles to determine if an access key needs
    ...
    def prompt_for_upload_s3_secret_key(self):
        """Should be overridden by profiles to determine if a secret key needs
        to be provided for upload or not
        """
    ...
    def prompt_for_upload_s3_bucket(self):
        """Should be overridden by profiles to determine if a bucket needs to
    ...
    def prompt_for_upload_s3_endpoint(self):
        """Should be overridden by profiles to determine if an endpoint needs
        to be provided for upload or not
        """
    ...
    def prompt_for_upload_user(self):
        """Should be overridden by profiles to determine if a user needs to
        be provided or not
        """
    ...
    def prompt_for_upload_password(self):
        """Should be overridden by profiles to determine if a password needs to
        be provided for upload or not
        """
    ...
    def upload_archive(self, archive):
        ...
        profile or user specified location.

        Currently there is support for HTTPS, SFTP, and FTP. HTTPS uploads are
        preferred for profile-defined defaults.

        Profiles that need to override uploading methods should override the
        ...
        In order to enable this for a profile, that profile needs to implement
        the following:
        ...
        The following Class Attrs may optionally be overidden by the Profile

        :_upload_directory:     Default FTP server directory, if any


        The following methods may be overridden by ``Profile`` as needed
        ...
        if not self.upload_url:
            raise Exception("No upload destination provided by upload profile"
                            " or by --upload-url")
    ...
    def get_upload_s3_access_key(self):
        """Helper function to determine if we should use the profile default
    ...
    def get_upload_url_string(self):
        """Used by upload profiles to potentially change the string used to
    ...
    def get_upload_password(self):
        ...
        environment variable will have precendent over any profile value
    ...
    def _get_sftp_upload_name(self):
        ...
        override this method in the relevant Upload Profile. Otherwise the
    ...
    def _upload_https_put(self, archive, verify=True):
        ...
        Profiles should override this method instead of the base upload_https()
    ...
    def upload_ftp(self, url=None, directory=None, user=None, password=None):
        """Attempts to upload the archive to either the profile defined or user
        ...
        if url is None:
            raise Exception("no FTP server specified by upload profile, "
